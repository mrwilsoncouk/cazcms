...
function writeHashed(name, content){ const ext=path.extname(name); const base=path.basename(name,ext); const file=`${base}.${hash(content)}${ext}`; fs.writeFileSync(path.join(ASSETS,file),content); return `/site/assets/${file}`; }
...
function productCard(p){ return `<div class="card"><h2><a href="/site/products/${esc(p.slug||p.sku||p.id)}.html">${esc(p.name)}</a></h2><p>${esc(p.description||'')}</p><strong class="price">${esc(p.currency||site.currency||'GBP')} ${p.salePrice||p.price||0}</strong></div>`; }
...
fs.writeFileSync(path.join(OUT,'index.html'),page(site.name,index,{description:site.description||''},[{label:'Home'}]));
for(const c of content){ fs.writeFileSync(path.join(OUT,`${c.slug}.html`),page(c.title,contentHtml(c),c.seo,[{label:'Home',href:'/site/index.html'},{label:c.title}])); }
fs.mkdirSync(path.join(OUT,'products'),{recursive:true});
fs.writeFileSync(path.join(OUT,'products.html'),page('Products',`<h1>Products</h1><div class="grid">${products.map(productCard).join('')}</div>`,{description:'Product catalogue'},[{label:'Home',href:'/site/index.html'},{label:'Products'}]));
for(const p of products){
 const related=products.filter(x=>x.id!==p.id).slice(0,4);
 fs.writeFileSync(path.join(OUT,'products',`${p.slug||p.sku||p.id}.html`),page(p.name,`<article><h1>${esc(p.name)}</h1><p>${esc(p.description||'')}</p><p class="price">${esc(p.currency||site.currency||'GBP')} ${p.salePrice||p.price||0}</p><section class="lazy-widget"><h2>Related products</h2><div class="grid">${related.map(productCard).join('')}</div></section></article>`,p.seo||{},[{label:'Home',href:'/site/index.html'},{label:'Products',href:'/site/products.html'},{label:p.name}]));
}
const sitemapUrls=[...content.map(c=>`/site/${c.slug}.html`),'/site/products.html',...products.map(p=>`/site/products/${p.slug||p.sku||p.id}.html`)];
fs.writeFileSync(path.join(OUT,'sitemap.xml'),`<?xml version="1.0"?><urlset>${sitemapUrls.map(loc=>`<url><loc>${loc}</loc><lastmod>${new Date().toISOString()}</lastmod></url>`).join('')}</urlset>`);
